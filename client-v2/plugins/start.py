...
_ALL_STYLES = [ButtonStyle.PRIMARY, ButtonStyle.SUCCESS, ButtonStyle.DANGER]

def random_styles(count):
    """Return `count` random ButtonStyles with no adjacent duplicates."""
    styles = []
    for _ in range(count):
        choices = [s for s in _ALL_STYLES if not styles or s != styles[-1]]
        styles.append(random.choice(choices))
    return styles
...
def start_markup():
    ...
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ", url="https://t.me/Team_SixtyNine", icon_custom_emoji_id=4958479549265347295, style=s[0])
        ],
        [
            InlineKeyboardButton("ᴍʏ ꜰɪʟᴇꜱ", callback_data="show_myfiles_1", icon_custom_emoji_id=6039630677182254664, style=s[1])
        ]
    ])
...
def build_myfiles_buttons(page, total_pages):
    ...
    if page > 1 and page < total_pages:
        ns = random_styles(2)
        nav.append(InlineKeyboardButton("Prev", callback_data=f"show_myfiles_{page-1}", icon_custom_emoji_id=5213358684024877471, style=ns[0]))
        nav.append(InlineKeyboardButton("Next", callback_data=f"show_myfiles_{page+1}", icon_custom_emoji_id=6206366856710719928, style=ns[1]))
    elif page > 1:
        nav.append(InlineKeyboardButton("Prev", callback_data=f"show_myfiles_{page-1}", icon_custom_emoji_id=5213358684024877471, style=random.choice(_ALL_STYLES)))
    elif page < total_pages:
        nav.append(InlineKeyboardButton("Next", callback_data=f"show_myfiles_{page+1}", icon_custom_emoji_id=6206366856710719928, style=random.choice(_ALL_STYLES)))

    back_style = random.choice([s for s in _ALL_STYLES if not nav or s != nav[-1].style]) if nav else random.choice(_ALL_STYLES)
    rows = []
    if nav:
        rows.append(nav)
    rows.append([InlineKeyboardButton("ʙᴀᴄᴋ ᴛᴏ ᴍᴇɴᴜ", callback_data="back_to_menu", icon_custom_emoji_id=5352759161945867747
, style=back_style)])
