...
async def process_file(client, message):
    ...
    if stream_url.startswith("https://"):
        ...
        cs = random.sample(_ALL_STYLES, 2)
        rm = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("sᴛʀᴇᴀᴍ", url=stream_url, icon_custom_emoji_id=4956324463525233747, style=cs[0]),
                InlineKeyboardButton("ᴅᴏᴡɴʟᴏᴀᴅ", url=download_url, icon_custom_emoji_id=6046627337821231012, style=cs[1])
            ]
        ])
        ...
        if not is_member:
            ...
            await message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("Join Channel", url=join_url, icon_custom_emoji_id=5370599459661045441, style=js[0])],
                    [InlineKeyboardButton("I've Joined", callback_data="force_join_check", icon_custom_emoji_id=6255871886714999384, style=js[1])],
                ]),
                quote=True,
            )
            return
